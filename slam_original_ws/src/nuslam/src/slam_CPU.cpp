...
#include <string>
#include <armadillo>
// [EKF LOGGER] headers needed by the summary logger
...
// ─────────────────────────────────────────────────────────────────────────
// [EKF LOGGER]
// Logger del resumen del benchmark.
//
// Acumula el tiempo de la operacion PHt (= sigma * Hj^T) en cada iteracion
// del update del EKF y, al destruirse el nodo (Ctrl+C -> rclcpp shutdown),
// escribe el CSV de resumen en la ruta indicada por la variable de entorno
// EKF_CSV_PATH (la que exporta run_benchmark.sh).
//
// La cabecera del CSV coincide exactamente con results_*_summary.csv:
//   mode,total_iterations,avg_op_us,min_op_us,max_op_us,std_op_us,
//   avg_sync_to_us,avg_kernel_us,avg_sync_from_us,
//   final_theta,final_x,final_y,final_sigma_theta,final_sigma_x,final_sigma_y,
//   final_lm0_x,final_lm0_y
//
// En la version armadillo (CPU) las columnas sync/kernel quedan en 0; la
// version FPGA reutiliza la MISMA clase pasando los tiempos reales de
// transferencia y kernel a record_op().
// ─────────────────────────────────────────────────────────────────────────
class EkfLogger
{
public:
  explicit EkfLogger(const std::string & mode)
  : mode_(mode)
  {
    const char * path = std::getenv("EKF_CSV_PATH");
    if (path != nullptr) {
      csv_path_ = path;
    }
  }

  // Se llama una vez por operacion PHt. op_us = tiempo total de la operacion.
  // sync_to/kernel/sync_from solo aplican en FPGA (default 0 en armadillo).
  void record_op(
    double op_us,
    double sync_to_us = 0.0,
    double kernel_us = 0.0,
    double sync_from_us = 0.0)
  {
    op_us_.push_back(op_us);
    sum_sync_to_ += sync_to_us;
    sum_kernel_ += kernel_us;
    sum_sync_from_ += sync_from_us;
  }

  // Guarda el estado final del estimador. Se sobreescribe en cada iteracion,
  // de modo que al apagar el nodo contiene el ultimo estado valido.
  void set_final_state(
    double theta, double x, double y,
    double sigma_theta, double sigma_x, double sigma_y,
    double lm0_x, double lm0_y)
  {
    final_theta_ = theta;
    final_x_ = x;
    final_y_ = y;
    final_sigma_theta_ = sigma_theta;
    final_sigma_x_ = sigma_x;
    final_sigma_y_ = sigma_y;
    final_lm0_x_ = lm0_x;
    final_lm0_y_ = lm0_y;
  }
  ...
  void write()
  {
    ...
    if (csv_path_.empty()) {
      std::cerr << "[EkfLogger] EKF_CSV_PATH no definida: no se escribe resumen\n";
      return;
    }
    if (op_us_.empty()) {
      std::cerr << "[EkfLogger] no se registraron operaciones: no se escribe resumen\n";
      return;
    }

    const std::size_t n = op_us_.size();
    double sum = 0.0;
    ...
    if (!f.is_open()) {
      std::cerr << "[EkfLogger] no se pudo abrir " << csv_path_ << "\n";
      return;
    }
    ...
    f << mode_ << ","
      << n << ","
      << avg << "," << mn << "," << mx << "," << stddev << ","
      << (sum_sync_to_ / static_cast<double>(n)) << ","
      << (sum_kernel_ / static_cast<double>(n)) << ","
      << (sum_sync_from_ / static_cast<double>(n)) << ","
      << final_theta_ << "," << final_x_ << "," << final_y_ << ","
      << final_sigma_theta_ << "," << final_sigma_x_ << "," << final_sigma_y_ << ","
      << final_lm0_x_ << "," << final_lm0_y_ << "\n";

    f.close();
    std::cerr << "[EkfLogger] resumen escrito en " << csv_path_
              << " (" << n << " iteraciones)\n";
  }
...
private:
  ...
  std::vector<double> op_us_;
  double sum_sync_to_ = 0.0;
  double sum_kernel_ = 0.0;
  double sum_sync_from_ = 0.0;
  double final_theta_ = 0.0;
  double final_x_ = 0.0;
  double final_y_ = 0.0;
  double final_sigma_theta_ = 0.0;
  double final_sigma_x_ = 0.0;
  double final_sigma_y_ = 0.0;
  double final_lm0_x_ = 0.0;
  double final_lm0_y_ = 0.0;
  bool written_ = false;
};
...
private:
  ...
  void fake_sensor_cb(const visualization_msgs::msg::MarkerArray & msg)
  {
    ...
    for (size_t i = 0; i < fake_sensor_data_.markers.size(); i++) {
      ...
      if (marker.action != visualization_msgs::msg::Marker::DELETE) {
        ...
        // eq 26
        // [EKF LOGGER] cronometrar la operacion PHt = sigma * Hj^T (lo que
        // ejecuta el kernel en la FPGA). Numericamente identico al original:
        //   Hj * sigma * Hj^T == Hj * PHt  y  sigma * Hj^T * X == PHt * X
        const auto t_op_0 = std::chrono::steady_clock::now();
        arma::mat PHt = sigma * Hj.t();
        const auto t_op_1 = std::chrono::steady_clock::now();
        ekf_logger_.record_op(
          std::chrono::duration<double, std::micro>(t_op_1 - t_op_0).count());

        arma::mat K = PHt * arma::inv(Hj * PHt + R);
    ...
    // [EKF LOGGER] guardar el estado final (se sobreescribe cada iteracion).
    // final_sigma_* = desviacion estandar = sqrt(diagonal de la covarianza).
    // Si tu version original guardaba la varianza cruda, quita el std::sqrt.
    ekf_logger_.set_final_state(
      state(0), state(1), state(2),
      std::sqrt(sigma(0, 0)), std::sqrt(sigma(1, 1)), std::sqrt(sigma(2, 2)),
      state(3), state(4));
  ...
  void sensor_cb(const visualization_msgs::msg::MarkerArray & msg)
  {
    ...
    for (size_t i = 0; i < msg.markers.size(); i++) {
      ...
      // eq 26
      // [EKF LOGGER] misma operacion PHt = sigma * Hj^T que en la ruta sim.
      const auto t_op_0 = std::chrono::steady_clock::now();
      arma::mat PHt = sigma * Hj.t();
      const auto t_op_1 = std::chrono::steady_clock::now();
      ekf_logger_.record_op(
    ...
    // [EKF LOGGER] guardar el estado final (igual que en la ruta sim).
    ekf_logger_.set_final_state(
      state(0), state(1), state(2),
      std::sqrt(sigma(0, 0)), std::sqrt(sigma(1, 1)), std::sqrt(sigma(2, 2)),
      state(3), state(4));
  ...
  arma::mat q_bar;
  arma::mat R;
  // [EKF LOGGER] instancia del logger. "armadillo" en la build CPU; la build
  // FPGA usa "fpga" (es lo que aparece en la columna mode del CSV).
  EkfLogger ekf_logger_{"armadillo"};
