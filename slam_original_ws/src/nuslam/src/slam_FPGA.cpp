...
public:
  ...
  : Node("slam")
  {
    ...
    auto xclbin_path = declare_parameter<std::string>("xclbin_path", "ekf_spmm.xclbin");
    accel_ = std::make_unique<EkfSpmmAccel>(xclbin_path);
...
{
  if (accel_) {
    accel_->write_summary("fpga", "results_fpga_summary.csv",
...
private:
  ...
  void fake_sensor_cb(const visualization_msgs::msg::MarkerArray & msg)
  {
    ...
    for (size_t i = 0; i < fake_sensor_data_.markers.size(); i++) {
      ...
      if (marker.action != visualization_msgs::msg::Marker::DELETE) {
        ...
        // eq 26
	// ── Volcado único de los inputs reales del EKF para verificar la FPGA ──
	static bool ekf_dumped = false;
	if (!ekf_dumped) {
	  sigma.save("ekf_sigma.txt", arma::arma_ascii);   // sigma predicha (n x n)
	  Hj.save("ekf_hj.txt", arma::arma_ascii);          // Jacobiano denso (2 x n)
	  RCLCPP_INFO_STREAM(get_logger(), "EKF inputs volcados: ekf_sigma.txt, ekf_hj.txt");
	  ekf_dumped = true;
	}
	arma::mat pht = accel_->compute_pht(sigma, Hj);
  ...
  arma::mat q_bar;
  arma::mat R;
  std::unique_ptr<EkfSpmmAccel> accel_;
